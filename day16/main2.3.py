    ...
    while heap:
        ...
        if state in visited:
            if cost > visited[state]:
                continue
            elif cost == visited[state]:
                # Already have predecessors for this state with the same cost
                pass
            else:
        ...
        # If reached the end, continue to process to find all optimal paths
        if (x, y) == end_pos:
        ...
        if 0 <= new_x < len(grid[0]) and 0 <= new_y < len(grid):
            ...
            if target_cell != '#':  # Can move forward
                ...
                if new_state not in visited or new_cost < visited.get(new_state, float('inf')):
                    heapq.heappush(heap, (new_cost, new_x, new_y, direction))
                    predecessors[new_state] = [state]  # Reset predecessors
                elif new_cost == visited.get(new_state, float('inf')):
                    heapq.heappush(heap, (new_cost, new_x, new_y, direction))
                    predecessors[new_state].append(state)  # Add additional predecessor
