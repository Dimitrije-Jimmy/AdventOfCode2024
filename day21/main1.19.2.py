...
# Corrected Directional Keypad Adjacency
directional_keypad_adjacency = {
    '^': {'^': None, 'v': 'v', '<': None, '>': 'A'},
    'v': {'^': '^', 'v': None, '<': '<', '>': '>'},
    '<': {'^': None, 'v': None, '<': None, '>': 'v'},
    '>': {'^': 'A', 'v': None, '<': 'v', '>': None},
    'A': {'^': None, 'v': None, '<': 'v', '>': None}
}

@lru_cache(maxsize=None)
def a_star_search(start_button, target_string, keypad_adjacency):
    """
    A* search to find all minimal sequences that produce the target_string on the keypad.
    ...
    while heap:
        ...
        for move_cmd in ['<', '^', 'v', '>']:
            neighbor = keypad_adjacency[current_btn].get(move_cmd)
...
def complexity(code, sequence):
    ...
    """
    # Extract numeric part by removing trailing 'A's
