...
def bfs_all_min_sequences(start_button, target_string, keypad_adjacency):
    ...
    while queue:
        ...
        # If we've already completed the target string, no need to expand further
        if idx == len(target_string):
            solutions.append(path)  # Add the valid path
            continue
        ...
        for move_cmd in ['<', '^', 'v', '>']:
            ...
            if neighbor is not None:
                ...
                if next_state not in dist or len(new_path) < dist[next_state]:
                    dist[next_state] = len(new_path)
    ...
    return solutions  # Return all paths that can produce the target string
