...
def bfs(grid, start, end, allow_cheat=False):
    ...
    queue = deque([(start[0], start[1], 0, False)])  # (x, y, cost, cheat_used)
    visited = set()
...
def dijkstra_with_cheats(grid, start, end, min_savings=1):
    """
    ...
    heap = []
    visited = set()
    cheats = []  # List of (start, end, time_saved)
    ...
    while heap:
        ...
        if (x, y) == end:
            if cheat_start is not None:
                ...
                time_saved = time_without_cheat - cost
                if time_saved >= min_savings:
                    cheats.append((cheat_start, (x, y), time_saved))
            continue
