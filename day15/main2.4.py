...
def simulate_moves_part2(walls: Set[Tuple[int, int]], boxes: Set[Tuple[int, int]], robot_pos: Tuple[int, int], moves: List[str]) -> Tuple[Set[Tuple[int, int]], Tuple[int, int]]:
    ...
    for move_num, move in enumerate(moves, start=1):
        ...
        if target_pos in walls:
            ...
            # Collect all contiguous boxes in the movement direction
            while current_pos in boxes:
                boxes_to_push.append(current_pos)
                current_pos = (current_pos[0] + dx, current_pos[1] + dy)

            # Determine the new positions for all boxes to be pushed
            new_box_positions = []
            can_push = True
            for box in boxes_to_push:
                ...
                # Since boxes are two-wide, check both positions
                ...
                if new_box_pos in walls or new_box_pos in boxes or right_part_new in walls or right_part_new in boxes:
                    ...
                    break
                new_box_positions.append((new_box_pos, right_part_new))
            ...
            for original_box, (new_box_pos, new_right_pos) in zip(boxes_to_push, new_box_positions):
                # Remove the original box
                boxes.remove(original_box)
                boxes.remove((original_box[0] + 1, original_box[1]))  # Remove the right part
                # Add the new box
                boxes.add(new_box_pos)
                boxes.add(new_right_pos)
                print(f"Move {move_num}: Pushed box from {original_box} to {new_box_pos} and {original_box[0]+1, original_box[1]} to {new_right_pos}.")
...
def calculate_gps_sum_part2(boxes: Set[Tuple[int, int]]) -> int:
    ...
    for (x, y) in boxes:
        ...
        # Since boxes are two-wide, we use the left x-coordinate
        gps = 100 * y + x
        gps_sum += gps
    return gps_sum
