...
def simulate_moves_part_two(walls: Set[Tuple[int, int]], boxes: Set[Tuple[int, int]], robot_pos: Tuple[int, int], moves: str) -> Tuple[Set[Tuple[int, int]], Tuple[int, int]]:
    """
    Simulates the robot's movements and box pushes for Part Two, handling double-wide boxes and multiple pushes per move.
    ...
    for move_num, move in enumerate(moves, start=1):
        ...
        while push_possible:
            ...
            if target_pos in walls:
                print(f"Move {move_num}: '{move}' blocked by wall at {target_pos}. Cannot push further.")
                push_possible = False
                break
                ...
                if current_pos in walls or current_pos in boxes:
                    print(f"Move {move_num}: Cannot push boxes in direction '{move}'. Blocked at {current_pos}.")
                    push_possible = False
                    break
                ...
                robot_pos = target_pos
                print(f"Move {move_num}: Robot moved '{move}' to {robot_pos}.")
                push_possible = False  # No box to push, so stop
