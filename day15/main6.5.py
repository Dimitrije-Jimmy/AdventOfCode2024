...
def calculate_gps_sum(tile_grid):
    gps_sum = 0
    for y, row in enumerate(tile_grid):
        for tx, tile in enumerate(row):
            if tile == '[]':
                # According to the puzzle, for the scaled map,
                # distances are measured from edges the same way as before.
                # If the puzzle states we need an offset, apply it:
                x_char = tx * 2
                # If puzzle's final example matches no offset:
                gps = 100 * y + x_char
                gps_sum += gps
    return gps_sum
