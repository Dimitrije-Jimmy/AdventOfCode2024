    ...
    for line in map_lines:
        ...
        for char in line:
            if char == '#':
                ...
                # If there are unexpected characters, treat them as empty spaces
                scaled_line += '..'
...
def visualize_grid_part_two(walls: Set[Tuple[int, int]], boxes: Set[Tuple[int, int]], robot_pos: Tuple[int, int]) -> None:
    ...
    Args:
        walls (set of tuples): Coordinates of walls.
        boxes (set of tuples): Coordinates of boxes (both parts).
        robot_pos (tuple): (x, y) coordinates of the robot.
    """
    ...
    for y in range(min_y, max_y + 1):
        ...
        while x <= max_x:
            ...
            if (x, y) in walls and (x+1, y) in walls:
                ...
                row.append('@.')
            elif (x, y) not in walls and (x, y) not in boxes and pos != robot_pos:
                row.append('..')
            else:
                # If any inconsiderate pair is found, represent it as '..' to avoid '??'
                row.append('..')
            x += 2  # Move to the next pair
...
def simulate_moves_part_two(walls: Set[Tuple[int, int]], boxes: Set[Tuple[int, int]], robot_pos: Tuple[int, int], moves: str) -> Tuple[Set[Tuple[int, int]], Tuple[int, int]]:
    ...
    for move_num, move in enumerate(moves, start=1):
        ...
        if target_pos in walls:
            ...
            # Since boxes are double-wide, ensure that we push both parts
            # Sort boxes_to_push based on movement direction to avoid overlapping issues
            if dx > 0 or dy > 0:
            ...
            for box_pos in boxes_to_push_sorted:
                ...
                # For boxes, ensure that the adjacent cell is also available
                if dx != 0:
                    # Horizontal movement, boxes are double-wide
                    if (new_box_pos in walls) or (new_box_pos in boxes):
                        can_push = False
                        break
                else:
                    # Vertical movement, boxes remain double-wide
                    if (new_box_pos in walls) or (new_box_pos in boxes):
                        can_push = False
                        break

            if not can_push:
                print(f"Move {move_num}: Cannot push boxes in direction '{move}'. Move failed.")
                continue
