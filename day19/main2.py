...
def count_arrangements(design, towel_patterns, memo):
    """
    Counts the number of ways a design can be formed using the available towel patterns.
    ...
    Returns:
        int: The total number of ways to form the design.
    """
    if design in memo:
        return memo[design]
    ...
    for pattern in towel_patterns:
        if design.startswith(pattern):
            remainder = design[len(pattern):]
            total_ways += count_arrangements(remainder, towel_patterns, memo)
...
def total_arrangements(towel_patterns, desired_designs):
    """
    Calculates the total number of arrangements for all desired designs.
    ...
    Returns:
        int: The total number of arrangements for all designs.
    """
    memo = {}
    total = 0
    for design in desired_designs:
        total += count_arrangements(design, towel_patterns, memo)
    return total
...
def main():
    ...
    result2 = total_arrangements(towel_patterns, desired_designs)
    print(f"Total number of arrangements: {result2}")
