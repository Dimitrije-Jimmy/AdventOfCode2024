...
def move_files(data):
    ...
    for file_id, file_start, file_size in files_sorted:
        if file_size == 0:
            continue  # Skip files with size 0
        ...
        # Check if the file is already within this span or overlaps with it
        file_end = file_start + file_size
        span_end = span_start + span_length
        if span_start <= file_start and span_end >= file_end:
            # The file is already within a suitable span; no move needed
            continue
        ...
        # Set the new position to the file ID
        for i in range(new_start, new_start + file_size):
            data[i] = str(file_id)
        # Set the old position to free space
