...
def execute_program(AInit, BInit, CInit, program):
    A = AInit
    B = BInit
    C = CInit
    IP = 0
...
def forward_check_with_constraints(program, constraints):
    """
    Given the program and a set of constraints on A_start,
    use the constraints to generate candidate A_start values 
    and check them by running the program forward.
    
    For this example, assume the constraints are simple and directly apply to A_start.
    For example: [('mod8_eq', 'A_start', 4)]
    This means A_start % 8 = 4.
    """

    # Convert the program to a target output string
    target_output = ",".join(map(str, program))

    # Extract constraints:
    # For simplicity, let's handle just one constraint: ('mod8_eq', 'A_start', value)
    # In a real scenario, you'd handle all constraints.
    mod_constraints = []
    for c in constraints:
        if c[0] == 'mod8_eq' and c[1] == 'A_start':
            mod_constraints.append(c[2]) # value

    # If we have a mod8_eq constraint for A_start:
    # Let's say we have only one for demonstration
    candidate_values = []
    if mod_constraints:
        mod_val = mod_constraints[0]  # e.g. 4
        # Generate candidates that satisfy A_start % 8 = mod_val
        # We'll try a small range to demonstrate. If needed, go larger or use heuristics.
        # Since we know brute force is huge, we rely on constraints to narrow it down.
        # Let's just try from A_start = mod_val up to mod_val+8*100 for a demo.
        for k in range(10000): # Try 10k candidates
            A_candidate = 8*k + mod_val
            candidate_values.append(A_candidate)
    else:
        # No constraints: we must still try something, but this is back to brute force
        # We'll just try a small range for demonstration.
        candidate_values = range(1, 100000)

    # Now check candidates:
    B = 0
    C = 0
    for A_candidate in candidate_values:
        out = execute_program(A_candidate, B, C, program)
        if out == target_output:
            print(f"Found a solution! A_start = {A_candidate}")
            return A_candidate
...
def main():
    program = [0,3,5,4,3,0]
    # Suppose the backward analysis gave us a constraint:
    # A_start % 8 = 4, represented as [('mod8_eq', 'A_start', 4)]
    constraints = backward_execute(program)
    print("Collected constraints:", constraints)
    # Then solve them:
    solve_constraints(constraints)
    #constraints = [('mod8_eq', 'A_start', 4)]

    # Now run forward checks with these constraints
    forward_check_with_constraints(program, constraints)
